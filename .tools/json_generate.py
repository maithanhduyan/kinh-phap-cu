...
# Đây là chuỗi SQL chứa lệnh INSERT dữ liệu vào bảng baike
sql_baike = """
...
# Đây là chuỗi SQL chứa lệnh INSERT dữ liệu vào bảng pham
sql_pham = """
...
# --------- Phân tích dữ liệu bảng baike ---------
# Bước 1: Tách phần chứa dữ liệu sau từ khóa VALUES và trước dấu ; cuối cùng.
baike_match = re.search(r"VALUES\s*(.+);", sql_baike, re.DOTALL)
if not baike_match:
    raise ValueError("Không tìm thấy phần VALUES trong SQL baike!")
values_str = baike_match.group(1)

# Bước 2: Tách từng bộ giá trị được bao bởi cặp dấu ngoặc ()
records = re.findall(r"\((.*?)\)(?:,|\n|$)", values_str, re.DOTALL)

# Danh sách các trường theo thứ tự trong câu lệnh INSERT của baike
keys_baike = ["id", "content", "number", "pham", "img_url", "created_at", "updated_at"]
...
for rec in records:
    ...
    fields = next(reader)
    # Loại bỏ khoảng trắng ở đầu và cuối từng trường
    fields = [field.strip() for field in fields]
    record_dict = dict(zip(keys_baike, fields))
    baike_data.append(record_dict)

# --------- Phân tích dữ liệu bảng pham ---------
pham_match = re.search(r"VALUES\s*(.+);", sql_pham, re.DOTALL)
if not pham_match:
    raise ValueError("Không tìm thấy phần VALUES trong SQL pham!")
pham_values_str = pham_match.group(1)

# Tách từng record của pham
pham_records = re.findall(r"\((.*?)\)(?:,|\n|$)", pham_values_str, re.DOTALL)
keys_pham = ["id", "name", "content", "created_at", "updated_at"]

pham_data = []
for rec in pham_records:
    f = io.StringIO(rec)
    reader = csv.reader(f, delimiter=',', skipinitialspace=True, quotechar="'")
    fields = next(reader)
    fields = [field.strip() for field in fields]
    record_dict = dict(zip(keys_pham, fields))
    pham_data.append(record_dict)

# Tạo từ điển mapping từ mã pham (trường "name") sang dữ liệu của pham
pham_dict = { item["name"]: item for item in pham_data }

# --------- Nối dữ liệu của pham vào từng record của baike ---------
# Với mỗi record trong baike, dùng trường "pham" làm khóa để tìm thông tin tương ứng trong pham_dict
for record in baike_data:
    pham_key = record.get("pham")
    if pham_key in pham_dict:
        # Thêm một trường mới "pham_details" chứa dữ liệu pham tương ứng
        record["category_name"] = pham_dict[pham_key].get("content")
    else:
        record["category_name"] = None

# Xuất kết quả ra JSON (hoặc có thể lưu vào file)
json_output = json.dumps(baike_data, ensure_ascii=False, indent=2)
print(json_output)
